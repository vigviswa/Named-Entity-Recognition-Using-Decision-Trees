...
### I am adding the Corpus Reader Class and Doing my part of the task

## Object Oriented Design Neeche
  ...
  # Adding a column of tokens to the dataframe
    ...
    def create_syn(self, dataframe, col):
      ...
      for i, val in enumerate(dataframe[col]):
        ...
        hypernym_e[i] = ', '.join(v for v in all_hyper)
      ...
      for i, val in enumerate(ans_df[col]):
        ...
        for word in string:
          if wn.synsets(word):
            ...
            # hype = syn.hypernyms()
            # hypo = syn.hyponyms()
            holo = wn.synsets(word)[0].part_holonyms()
            # mero = syn.part_meronyms()
      ...
      for i, val in enumerate(ans_df[col]):
        ...
        for word in string:
          if wn.synsets(word):
            ...
            # hypo = syn.hyponyms()
            # holo = syn.part_holonyms()
            mero = wn.synsets(word)[0].part_meronyms()
...
# all_hypo = []
  ...
  for i, val in enumerate(ans_df['filtered tokens']):
    val = val.replace(' ', '')
    string = val.split(',')
    for word in string:
      if wn.synsets(word):
